...
;(function($) {
		...
		init: function(options) {
			return this.each(function() {
				...
				for (var i = 1; i <= opt.number; i++) {
					...
					var timg = $('<img class="reatyimg" src="' + opt.path + starFile + '" alt="' + i + '" title="' + hint + '" />');
					$this.append(timg);
					if (opt.space) {
					...
					//提示框
					//$this.append('<div class="ratytitle"><div class="ratytitle-til">'+i+'分 一般</div><div>回复问题很慢，态度一般。谈不上沟通顺畅。</div></div>');
	...
	$.fn.raty.defaults = {
		...
		half:			false,
		halfShow:		true,
		hintList:		['很差', '还行', '一般', '很好', '非常好'],
		iconRange:		undefined,
		noRatedMsg:		'not rated yet',
		number:			5,
		path:			'http://42.96.144.97:4433/static/Alliance/Public/Js/jQuery/jquery.raty/img/',
		precision:		false,
